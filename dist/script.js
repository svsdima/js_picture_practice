...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    /* Только при заполнении двух пунктов выводится стоимость, если есть промокод, получает скидку 30% */
...
var forms = function forms() {
  ...
  /* Инпут нужен, чтобы очищать инпуты после отправки на сервер */
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var name = item.files[0].name.split('.')[0].substring(0, 10) + dots + arr[1];
...
var scrolling = function scrolling(upSelector) {
  ...
  /* Появление и исчезание элемента */
